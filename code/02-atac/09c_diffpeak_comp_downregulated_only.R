...
plotdir <- "../../plots/02-atac/09/diff_peak_comp_downonly"
outdir <- "../../output/02-atac/09/diff_peak_comp_downonly"
...
for (i in seq_along(motif_code_ls)){
  ...
  if (!file.exists(vennlist_file)){
    a <- fread(Sys.glob(sprintf("%s/deseq_*%s_vs_*GFP_1_filteredConsensus_full.tsv", deseq_dir, TF_dosages[1]))) %>% dplyr::filter((log2FoldChange < -log2fc_cutoff) & (padj<padj_cutoff))
    b <- fread(Sys.glob(sprintf("%s/deseq_*%s_vs_*GFP_1_filteredConsensus_full.tsv", deseq_dir, TF_dosages[2]))) %>% dplyr::filter((log2FoldChange < -log2fc_cutoff) & (padj<padj_cutoff))
    c <- fread(Sys.glob(sprintf("%s/deseq_*%s_vs_*GFP_1_filteredConsensus_full.tsv", deseq_dir, TF_dosages[3]))) %>% dplyr::filter((log2FoldChange < -log2fc_cutoff) & (padj<padj_cutoff))
    d <- fread(Sys.glob(sprintf("%s/deseq_*%s_vs_*GFP_1_filteredConsensus_full.tsv", deseq_dir, TF_dosages[4]))) %>% dplyr::filter((log2FoldChange < -log2fc_cutoff) & (padj<padj_cutoff))
    e <- fread(Sys.glob(sprintf("%s/deseq_*%s_vs_*GFP_1_filteredConsensus_full.tsv", deseq_dir, TF_dosages[5]))) %>% dplyr::filter((log2FoldChange < -log2fc_cutoff) & (padj<padj_cutoff))
...
# 
  ...
  p16 <- ggplot(df, aes(x = score, fill=group, color=group)) +
    geom_density(adjust = 2, alpha=0.3)  + xlab(paste0(motif_code, " motif score")) +
  ...
  p17 <- ggplot(dfwithmotif, aes(x = score, fill=group, color=group)) +
    geom_density(adjust = 2, alpha=0.3)  + xlab(paste0(motif_code, " motif score (nonzero only)")) +
  ...
  p18 <- ggplot(df, aes(x = count, fill=group, color=group)) +
    geom_density(adjust = 3, alpha=0.3) + xlim(c(-1, 10)) + xlab(paste0(motif_code, " motif count")) +
  ...
  p19 <- ggplot(dfwithmotif, aes(x = count, fill=group, color=group)) +
    geom_density(adjust = 3, alpha=0.3) + xlim(c(-1, 10)) + xlab(paste0(motif_code, " motif count (nonzero only)")) +
...
df_list <- list("all_means"=all_means,
              "all_means_noLEF1_NR4A1_ELF1_PRDM1"=all_means[!all_means$TF %in% c("LEF1", "NR4A1", "ELF1", "PRDM1"),],
              "all_means_noKLF1_KLF4_SP4"=all_means[!all_means$TF %in% c("KLF1", "KLF4", "SP4"),],
              "all_means_noLEF1_NR4A1_ELF1_PRDM1_KLF1_KLF4_SP4"=all_means[!all_means$TF %in% c("KLF1", "KLF4", "SP4","LEF1", "NR4A1", "ELF1", "PRDM1"),],
              "all_means_noELF1_SP4"=all_means[!all_means$TF %in% c("ELF1", "SP4"),]
              )
...
for (i in seq_along(df_list)){
  ...
  pdf(paste0(plotdir, "/", names(df_list)[i], ".pdf"), height=6, width=6)
  print(lapply(paste0("p",1:24), function(n){eval(parse(text=n))}))
  dev.off()
}
