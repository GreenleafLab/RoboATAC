...
def parse_inputs():
    ...
    return [args.motifs, args.motif_starts, args.name, args.exp_ls, args.outdir, args.plotdir]
...
# ## parse inputs
motifs, motif_starts, name, exp_ls, outdir, plotdir = parse_inputs()
...
for experiment in exp_ls:
    ...
    df2 = pd.DataFrame([base_pred_logits_kd, synth_pred_logits_kd],
                    index=[f"base_pred_avglogits_{experiment}", f"synth_pred_avglogits_{experiment}"]).transpose()

    output.append(df)
    output_logits.append(df2)
